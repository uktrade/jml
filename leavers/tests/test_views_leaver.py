...
class TestLeaverDetailsMixin(TestCase):
    ...
    @mock.patch("leavers.views.leaver.search_people_finder", return_value=[people_finder_result])
    def test_get_leaver_details_with_result(self, mock_search_people_finder):
        ...
        self.assertEqual(leaver_details["first_name"], self.people_finder_result["first_name"])
        self.assertEqual(leaver_details["last_name"], self.people_finder_result["last_name"])
        self.assertEqual(leaver_details["grade"], self.people_finder_result["grade"])
        self.assertEqual(leaver_details["personal_phone"], self.people_finder_result["primary_phone_number"])
        self.assertEqual(leaver_details["work_email"], self.people_finder_result["email"])
        self.assertEqual(leaver_details["job_title"], self.people_finder_result["roles"][0]["job_title"])
        self.assertEqual(leaver_details["team_name"], self.people_finder_result["roles"][0]["team"]["name"])
